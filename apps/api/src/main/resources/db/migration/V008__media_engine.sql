-- ============================================================================
-- Phase 19 — Media Engine & Watermarks Schema (PostgreSQL)
-- ============================================================================
...
ALTER TABLE studio_watermark_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE studio_watermark_settings FORCE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS tenant_isolation_studio_watermark_settings ON studio_watermark_settings;
CREATE POLICY tenant_isolation_studio_watermark_settings ON studio_watermark_settings
    FOR ALL
    USING (
...
DROP POLICY IF EXISTS tenant_isolation_upload_intents ON upload_intents;
CREATE POLICY tenant_isolation_upload_intents ON upload_intents
    FOR ALL
    USING (
        studio_id = NULLIF(current_setting('app.current_studio_id', true), '')::uuid
        OR current_setting('app.is_admin', true) = 'true'
...
DROP POLICY IF EXISTS tenant_isolation_media_derivatives ON media_derivatives;
CREATE POLICY tenant_isolation_media_derivatives ON media_derivatives
    FOR ALL
    USING (
        studio_id = NULLIF(current_setting('app.current_studio_id', true), '')::uuid
        OR current_setting('app.is_admin', true) = 'true'
