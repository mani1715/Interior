-- ============================================================================
-- Phase 22 — AI Reference Image System Schema (PostgreSQL 18)
-- ============================================================================
...
ALTER TABLE ai_reference_metadata ENABLE ROW LEVEL SECURITY;
ALTER TABLE ai_reference_metadata FORCE ROW LEVEL SECURITY;

ALTER TABLE ai_job_references ENABLE ROW LEVEL SECURITY;
ALTER TABLE ai_job_references FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation_ai_reference_metadata ON ai_reference_metadata;
CREATE POLICY tenant_isolation_ai_reference_metadata ON ai_reference_metadata
    FOR ALL
    USING (
...
DROP POLICY IF EXISTS tenant_isolation_ai_job_references ON ai_job_references;
CREATE POLICY tenant_isolation_ai_job_references ON ai_job_references
    FOR ALL
    USING (
        studio_id = NULLIF(current_setting('app.current_studio_id', true), '')::uuid
        OR current_setting('app.is_admin', true) = 'true'
