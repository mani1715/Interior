...
- **PHASE 20 — PASS:** SEO Engine (Production Implementation). Studio SEO settings, dynamic sitemap generation, robots.txt management, canonical URL computation, SEO health diagnostics, search appearance overrides, and breadcrumbs. Flyway migration `V009__seo_engine.sql`.
- **PHASE 21 — PASS:** AI Visualizer Foundation (Production Implementation). Production AI visualization engine with provider abstraction (`AiImageProvider`), monthly quota tracking (50 free/mo), prompt validation/clamps, private asset ingestion, and mandatory legal disclaimers. Flyway migration `V010__ai_visualizer_foundation.sql`.
- **PHASE 21.1 — PASS:** AI Concept Privacy & Publication Hardening. Core architectural invariant: AI generation success != publication. All new generated AI assets default to `MediaType.AI_CONCEPT` and `MediaVisibility.PRIVATE`.
- **PHASE 22 — PASS:** AI Reference Image System (Production Implementation). Multi-modal reference guidance with 5 reference purposes (`STYLE_TRANSFER`, `MATERIAL_INSPIRATION`, `COLOR_PALETTE`, `FURNITURE_REPRESENTATION`, `LAYOUT_INSPIRATION`), structure preservation toggle, and up to 4 reference images per job. Flyway migration `V011__ai_reference_images.sql`.
...
> The documented canonical roadmap and implementation ledger in this file and `docs/PHASE_00_24_MASTER_AUDIT.md` is the authoritative source of truth.
> Phase numbers and migration numbers are decoupled by design (e.g., Phase 18 uses V007).
